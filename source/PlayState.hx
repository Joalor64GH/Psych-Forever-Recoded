...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		judgementCounter.setFormat(Paths.font("vcr.ttf"), 30, FlxColor.WHITE, FlxTextAlign.LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		botplayTxt = new FlxText(400, timeBarBG.y + 55, FlxG.width - 800, "Skill Issue Mode", 32);
		...
		engineVerwatermark = new FlxText(4, engineshitYPos, 0, "Psych Forever Recoded BETA v" + MainMenuState.psychforeverVersion + " (PE v" + MainMenuState.psychEngineVersion + ")", 12);
		engineVerwatermark.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.WHITE, FlxTextAlign.RIGHT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		engineVerwatermark.scrollFactor.set();
		engineVerwatermark.x = 855;
	...
	override public function update(elapsed:Float)
	{
		...
		if(ratingName == '?') {
			scoreTxt.text = 'Score: ' + songScore + ' // Combo Breaks: ' + songMisses + ' (' + (shits + songMisses) + ') // Rank: F (nan%)';
		} else {
			scoreTxt.text = 'Score: ' + songScore + ' // Combo Breaks: ' + songMisses + ' (' + (shits + songMisses) + ') // Rank: ' + ratingName + ' (' + Highscore.floorDecimal(ratingPercent * 100, 2) + '%)' + ' | ' + ratingFC;//peeps wanted no integer rating
		}
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		if (cpuControlled)
			daRating = "sick-perfect"; // MFC no matter what
		...
		switch (daRating)
		{
			...
			case "sick-perfect": // sick but awesome
				totalNotesHit += 1;
				note.ratingMod = 1;
				sickrows++;
				health += 0.025 * healthGain;
		...
		if((daRating == 'sick' || daRating == 'sick-perfect') && !note.noteSplashDisabled)
		{
	...
	function opponentNoteHit(note:Note):Void
	{
		...
		if (!note.isSustainNote)
		{
			...
			if(!note.noteSplashDisabled && ClientPrefs.opponentsplash)
			{
	...
	function spawnNoteSplashOnNote2(note:Note) {
		if(ClientPrefs.noteSplashes && note != null) {
			var strum2:StrumNote = opponentStrums.members[note.noteData];
			if(strum2 != null) {
				spawnNoteSplash(strum2.x, strum2.y, note.noteData, note);
			}
		}
	}
	...
	public function RecalculateRating() {
		...
		if(ret != FunkinLua.Function_Stop)
		{
			...
			if (songMisses >= 65) ratingFC = 'Skill Issue'; // haha you suk
		}
